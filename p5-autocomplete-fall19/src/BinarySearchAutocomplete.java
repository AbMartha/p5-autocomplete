	...
	public List<Term> topMatches(String prefix, int k) {
		...
		int first = firstIndexOf(myTerms, dummy, comp); 
	     ...
	     if(first == -1)
	        ...
	        ArrayList<Term> match = new ArrayList<>(Arrays.asList(Arrays.copyOfRange(myTerms, first, last+1))); //grab entries
	        Collections.sort(match, Comparator.comparing(Term::getWeight).reversed()); //sort by reverse weight
